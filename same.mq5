...
input double patrax_lot_size = 0.05;
input int patrax_take_profit = 30;
...
input double lot_size_high = 0.1;
input double lot_size_low = 0.01;
input int take_profit_high = 50;
input int take_profit_low = 20;
...
int OnInit(){
    ...
    panel_handle = ObjectCreate(0, "panel", OBJ_LABEL, 0, 0);
    ObjectSetText(panel_handle, "Trade Signals", 10);
...
void EnterTrades(string timeframe, double lot_size, int take_profit){
    ...
    if (inTrade) {
        ...
        if (CheckMovingAverageBreak(timeframe, ma_period_90, ma_period_250)) {
            ...
            if (tradeType == OP_BUY) {
                ticket = OrderSend(_Symbol, OP_BUY, lot_size, Ask, 3, 0, 0, "Buy Order", 0, 0, clrGreen);
                OrderModify(ticket, Ask, 0, take_profit, 0, clrGreen);
            } else if (tradeType == OP_SELL) {
                ticket = OrderSend(_Symbol, OP_SELL, lot_size, Bid, 3, 0, 0, "Sell Order", 0, 0, clrRed);
                OrderModify(ticket, Bid, 0, take_profit, 0, clrRed);
            }
...
void DisplayPanelSignals(){
    ...
    ObjectSetText(panel_handle, signal_text, 10);
}
